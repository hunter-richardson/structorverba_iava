...
/**
 * Classis {@link Temporis} temporem fōrmat scrībī.
 */
@SuppressWarnings({ "SpellCheckingInspection", "unused" })
final class Temporis {
  ...
  private Temporis(final long instans) {
    tempus = ZonedDateTime.ofInstant(Instant.ofEpochMilli(instans), ZoneId.systemDefault());
    punctillaPraecidemne = false;
  }

  private Temporis(@NonNull final Instant instans) {
    tempus = ZonedDateTime.ofInstant(instans, ZoneId.systemDefault());
    punctillaPraecidemne = false;
  }

  private Temporis(final boolean punctillaPraecidemne) {
    tempus = ZonedDateTime.now();
    this.punctillaPraecidemne = punctillaPraecidemne;
  }

  private Temporis(final long instans, final boolean punctillaPraecidemne) {
    tempus = ZonedDateTime.ofInstant(Instant.ofEpochMilli(instans), ZoneId.systemDefault());
    this.punctillaPraecidemne = punctillaPraecidemne;
  }

  private Temporis(@NonNull final Instant instans, final boolean punctillaPraecidemne) {
    tempus = ZonedDateTime.ofInstant(instans, ZoneId.systemDefault());
    this.punctillaPraecidemne = punctillaPraecidemne;
  }
  ...
  @NonNull public static String formem(final long tempus) {
    return new Temporis(tempus).formo();
  }
  ...
  @NonNull public static String formem(final long tempus, final boolean punctillaPraecidemne) {
    return new Temporis(tempus, punctillaPraecidemne).formo();
  }
  ...
  @NonNull public static String formem(final boolean punctillaPraecidemne) {
    return new Temporis(punctillaPraecidemne).formo();
  }
